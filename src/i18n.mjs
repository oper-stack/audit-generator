...
const RU = {
  ...
  sitemap: {
    ...
    // Обход больших вложенных карт останавливается ограничителем, и по-русски это надо сказать
    // словами, иначе прочитанная часть выглядит как полное измерение.
    value: (v) => {
      const n = nums(v);
      if (/read from/.test(v)) return `${plural(Number(n[0]), 'адрес', 'адреса', 'адресов')} прочитано из ${n[1]} файлов карты, всего файлов ${n[2]}`;
      if (/URL\(s\) in/.test(v)) return `${plural(Number(n[0]), 'адрес', 'адреса', 'адресов')} в карте сайта`;
      if (/not found|missing/i.test(v)) return 'не найдена';
      return `не отдаётся, код ${n[0]}`;
    },
  },
  ...
  canonical: { label: 'Канонический адрес', value: (v) => (/missing|none/i.test(v) ? 'не указан' : /of \d+ sampled/.test(v) ? `указан на ${nums(v)[0]} из ${nums(v)[1]} страниц выборки` : 'указан'), comment: (c) => (/missing on/.test(c) ? `нет на: ${c.replace(/^missing on:\s*/, '')}` : '') },
  // Добавлено 17.09.2026 вместе с новыми проверками. Русский отчёт платный, английские строки в нём
  // это такой же брак, как русский отчёт англоязычному покупателю.
  'title-all': { label: 'Заголовок на страницах выборки', value: (v) => `есть на ${nums(v)[0]} из ${nums(v)[1]} страниц выборки`, comment: (c) => (/missing on/.test(c) ? `нет на: ${c.replace(/^missing on:\s*/, '')}` : '') },
  'description-all': { label: 'Описание на страницах выборки', value: (v) => `есть на ${nums(v)[0]} из ${nums(v)[1]} страниц выборки`, comment: (c) => (/missing on/.test(c) ? `нет на: ${c.replace(/^missing on:\s*/, '')}` : '') },
  'schema-all': { label: 'Разметка на страницах выборки', value: (v) => `есть на ${nums(v)[0]} из ${nums(v)[1]} страниц выборки`, comment: (c) => (/missing on/.test(c) ? `нет на: ${c.replace(/^missing on:\s*/, '')}` : '') },
  'og-title': { label: 'og:title на страницах выборки', value: (v) => `есть на ${nums(v)[0]} из ${nums(v)[1]} страниц выборки`, comment: (c) => (c ? 'без него при пересылке ссылки видно голый адрес или чужой заголовок' : '') },
  'og-image': { label: 'og:image на страницах выборки', value: (v) => `есть на ${nums(v)[0]} из ${nums(v)[1]} страниц выборки`, comment: (c) => (c ? 'ссылку без картинки пролистывают в любом мессенджере' : '') },
  'twitter-card': { label: 'Карточка Twitter', value: (v) => (/no twitter/.test(v) ? 'не задана' : v.replace('twitter:card', 'тип')), comment: (c) => (c ? 'на неё опираются X и часть читалок, когда Open Graph неполный' : '') },
  'breadcrumbs-schema': { label: 'Разметка хлебных крошек', value: (v) => (/not found/.test(v) ? 'не найдена в выборке' : `есть на ${pages(nums(v)[0])} выборки`), comment: (c) => (c ? 'крошки говорят системе, где страница стоит, и показываются в ответе рядом со ссылкой' : '') },
  'howto-schema': { label: 'Разметка пошаговых инструкций', value: (v) => (/no step-by-step/.test(v) ? 'пошаговых страниц в выборке нет' : `есть на ${pages(nums(v)[0])} выборки`), comment: (c) => (c ? 'ставить только там, где страница правда ведёт по шагам: разметить ею что-то другое значит соврать системе' : '') },
  'product-schema': { label: 'Разметка товара', value: (v) => (/no product/.test(v) ? 'страниц товара в выборке нет' : `есть на ${pages(nums(v)[0])} выборки`), comment: (c) => (c ? 'только для страниц, которые правда продают названную вещь с ценой' : '') },
  'redirect-chain': { label: 'Переадресации до главной', value: (v) => (/without a redirect/.test(v) ? 'адрес отвечает сразу, без переадресаций' : `${plural(Number(nums(v)[0]), 'переадресация', 'переадресации', 'переадресаций')} до ответа`), comment: (c) => (c ? 'направьте первый адрес сразу на конечный: каждый прыжок стоит времени и веса ссылки' : '') },
  favicon: { label: 'Значок сайта', value: (v) => (/declared/.test(v) ? 'объявлен в шапке страницы' : /answers 200/.test(v) ? '/favicon.ico отвечает' : 'не объявлен, и /favicon.ico не отвечает'), comment: (c) => (c ? 'вкладка, закладка и карточка при пересылке ссылки останутся с пустым квадратом' : '') },
  'engine-chatgpt': { label: 'ChatGPT может читать сайт', value: (v) => (/blocked/.test(v) ? `закрыт в robots.txt: ${v.replace(/^blocked in robots.txt:\s*/, '')}` : 'открыт'), comment: (c) => (c ? 'ChatGPT не заберёт страницу, когда она понадобится для ответа, и не процитирует вас даже там, где вы лучший ответ' : '') },
  'engine-perplexity': { label: 'Perplexity может читать сайт', value: (v) => (/blocked/.test(v) ? `закрыт в robots.txt: ${v.replace(/^blocked in robots.txt:\s*/, '')}` : 'открыт'), comment: (c) => (c ? 'Perplexity не заберёт страницу, когда она понадобится для ответа' : '') },
  'engine-claude': { label: 'Claude может читать сайт', value: (v) => (/blocked/.test(v) ? `закрыт в robots.txt: ${v.replace(/^blocked in robots.txt:\s*/, '')}` : 'открыт'), comment: (c) => (c ? 'Claude не заберёт страницу, когда она понадобится для ответа' : '') },
  'engine-copilot': { label: 'Bing Copilot может читать сайт', value: (v) => (/blocked/.test(v) ? `закрыт в robots.txt: ${v.replace(/^blocked in robots.txt:\s*/, '')}` : 'открыт'), comment: (c) => (c ? 'Copilot не заберёт страницу, когда она понадобится для ответа' : '') },
  'engine-gemini': { label: 'Gemini может читать сайт', value: (v) => (/blocked/.test(v) ? `закрыт в robots.txt: ${v.replace(/^blocked in robots.txt:\s*/, '')}` : 'открыт'), comment: (c) => (c ? 'Gemini не заберёт страницу, когда она понадобится для ответа' : '') },
  'engine-apple': { label: 'Apple и DuckDuckGo могут читать сайт', value: (v) => (/blocked/.test(v) ? `закрыт в robots.txt: ${v.replace(/^blocked in robots.txt:\s*/, '')}` : 'открыт'), comment: (c) => (c ? 'их ассистенты не заберут страницу, когда она понадобится для ответа' : '') },
  ...
  'llms-health': { label: 'Страницы из llms.txt', value: (v) => (/^all /.test(v) ? `все ${nums(v)[0]} проверенных страниц отвечают и открыты для индексации` : v.replace(/(\d+) dead/, '$1 мёртвых').replace(/(\d+) redirecting/, '$1 с переадресацией').replace(/(\d+) closed to indexing/, '$1 закрытых от индексации').replace(/ of (\d+) checked: /, ' из $1 проверенных: ')), comment: (c) => (c ? 'ИИ читает этот файл вместо обхода сайта: каждая мёртвая или закрытая запись это страница, которую он процитирует неверно или не процитирует вовсе' : '') },
  'agent-card': { label: 'Карточка агента (/.well-known/agent.json)', value: (v) => (/valid JSON/.test(v) ? 'есть, и это правильный JSON' : /not JSON/.test(v) ? 'есть, но это не JSON' : 'нет'), comment: (c) => (c ? 'ранний и необязательный стандарт: этот файл читают каталоги ИИ-агентов, чтобы понять, кто вы и что предлагаете' : '') },
  'api-catalog': { label: 'Каталог API (RFC 9727)', value: (v) => (/^present/.test(v) ? 'есть' : 'нет'), comment: (c) => (c ? 'необязательно, и нужно только если у бизнеса действительно есть API, который агент мог бы вызвать' : '') },
  'agent-markdown': { label: 'Markdown-версия страниц', value: (v) => (/Link header/.test(v) ? 'предлагается через заголовок Link' : /next to the page/.test(v) ? 'лежит рядом со страницей' : 'не предлагается'), comment: (c) => (c ? 'агент, который просит text/markdown, получает HTML и вынужден его разбирать: отдать markdown дёшево и снимает догадки' : '') },
