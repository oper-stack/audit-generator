...
export const FIX_ACTIONS = {
  ...
  'llms-health': { scope: 'always', action: { ru: 'Пересобрать llms.txt по живым страницам: убрать мёртвые адреса, заменить переадресованные на конечные, исключить закрытые от индексации', en: 'Rebuild llms.txt from the live pages: drop dead URLs, replace redirecting ones with their destination, leave out anything closed to indexing' } },
