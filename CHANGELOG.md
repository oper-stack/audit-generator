...
### Changed
- **Обложки отдельным листом больше нет.** Заголовок уменьшен с 32 до 20 пунктов и стоит шапкой первого листа, четыре поля выстроены в строку вместо столбика. Лист, на котором было только название, в отчёте никто не читал.
- **Каждый лист теперь влезает в лист.** Переливали пять разделов: обзор, техника, AEO с GEO, ссылки с конверсией и сводка. Перелив рождал листы с тремя строчками и восемьюдесятью процентами белого. Длинные таблицы режутся по счёту строк с подписью «1/2», разделы, стоявшие по два на листе, разведены, а источники и границы замера сведены на последний лист к дорожной карте.
- **Отчёт стал ровно двенадцать листов** вместо тринадцати, из которых четыре были огрызками. Проверяется замером: перелив по каждому листу ноль.

## 0.30.0

### Changed
- **Платный отчёт говорит тем же языком, что и страница результата.** Сводка это кольцо со шкалой, две пилюли с числами, оценка словом, готовность по пяти движкам карточками и пять областей полосами с цветом по значению. Пороги цвета те же, что у буквы: 80 и 45. До этого бесплатная страница выглядела лучше платного отчёта, и владелец сказал об этом прямо.
- **Сводка занимает два полных листа**, а не один с переливом. Перелив оставлял лист с тремя строчками и восемьюдесятью процентами белого.
- **Длинная техническая таблица режется по листам** с подписью «1/2», а не переливается сама.
- **Лист «что чинить первым» не печатается, когда чинить нечего.** Раньше на чистом сайте он занимал целую страницу одной строкой.

### Added
- Готовность по движкам и основы SEO в платном отчёте: берутся из той же проверки, что уже посчитана, без единого лишнего запроса.
- В таблице прочитанных страниц появились H2, таблицы и ссылки.

### Fixed
- **Обращения к аналитику больше не уезжают покупателю.** «Подтвердите, что это верно» писалось для человека, который правит черновик, а в автоматическом товаре уходило клиенту как есть. Тест сторожит, чтобы не вернулось.
- **Русский отчёт по русскому аудиту больше не смешивает языки.** Названия областей переводились только тогда, когда аудит собран по-английски, и в русский текст уезжало «Хорошо сделано вот что: SEO, technical...». Переводим по языку отчёта.

## 0.29.0

### Added
- **Основы SEO, второе число рядом с ИИ-видимостью.** Считается только из уже прочитанных десяти страниц: заголовок, описание, H1, canonical, og:title, запрет индексации. Ни одного нового запроса. Называется «основы», потому что полный аудит смотрит на скорость, ссылки и позиции, и этого здесь нет. На балл ИИ-видимости не влияет: отдельное число, чтобы разрыв «для Google в порядке, для ChatGPT нет» был виден сразу.

## 0.28.0

### Changed
- **Готовность по движкам считается ТОЙ ЖЕ формулой, что и общий балл.** В 0.27.0 я дал двум группам движков разные веса: у тех, кто достаёт страницу живьём, тяжелее доступ, у тех, кто отвечает из индекса, тяжелее разметка. Обоснование звучало складно, но замером не было, то есть это было допущение, ровно то, за что мы критикуем конкурента. Теперь веса взяты один в один из балла, а отличаются движки только одним и настоящим: что robots.txt отвечает их собственным роботам. У сайта, который никого не закрыл, все пять чисел совпадут с баллом.
- Под числом вместо рассуждения о природе движка стоит факт: чьи роботы спрошены.
- Доля llms.txt тоже вошла в расчёт, потому что она входит в балл.

## 0.27.0

### Added
- **Готовность по каждому движку отдельно.** ChatGPT, Perplexity, Claude, Gemini и Copilot получают своё число. Ни одного нового замера: число пересобирается из того, что уже измерено, и рядом видно, из чего оно сложилось, какие роботы этого движка закрыты и какая область тянет вниз. Конкурент показывает такие числа без объяснения, откуда они, и мы за это его же и критикуем, поэтому веса названы в одном месте в коде и объяснены словами. Веса разные у двух групп: ChatGPT, Perplexity и Claude достают страницу живьём, поэтому у них решает доступ их собственным роботам, а Gemini и Copilot отвечают из чужого индекса, поэтому у них сильнее весит разметка.
- **Свойства каждой прочитанной страницы отдаются наружу:** H1, число подзаголовков, таблиц, ссылок и картинок. Заголовок, H1, подзаголовки и таблицы считались и раньше, но в ответ не попадали, и человек видел «19 из 25», не видя, из чего это сложилось.

### Unchanged
- Балл не тронут ни на один пункт: обе правки только показывают уже измеренное.
